...
const brand = document.querySelector(".brands-list");
let num = 0;
let wid = 0;

createList("logo.json");
setInterval(move, 50);
...
function createList(url) {
  fetch(url)
    ...
    .then((json) => {
      ...
      brand.innerHTML = tags;
      initList();
    });
}

function initList() {
  const listLi = brand.querySelectorAll("li");
  const len = listLi.length;
  const wid = parseInt(getComputedStyle(listLi[0]).width);

  brand.style.width = len * wid + "px";
  brand.style.marginLeft = -wid + "px";
  console.log(wid);
  console.log(num);
  brand.prepend(brand.lastElementChild);
}
...
function move() {
  if (num > -wid * 2) {
    ...
    brand.append(brand.firstElementChild);
  } else {
    num -= 2;
  }

  brand.style.marginLeft = num + "px";
}
